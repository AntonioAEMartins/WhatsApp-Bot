import { ConversationDto, ConversationStep, FeedbackDTO, PaymentProofDTO, PaymentStatus, UpdateConversationDto } from "src/conversation/dto/conversation.dto";
import { Message } from "whatsapp-web.js";
import { formatToBRL } from "./currency.utils";

/**
 * Step 0: Extract the order ID from the user's message.
 * Handles the initial processing of an order based on the user's message.
 *
 ...
 * @param message - The message object containing metadata and utilities.
 * @returns A Promise that resolves when the order processing is completed or the conversation state is updated.
 * 
 * Functionality: 
 * - Extracts the order ID from the user's message.
 * - Checks if the order is already being processed by another user.
 * - Updates the conversation state to reflect the current processing step.
 * - Sends appropriate messages to the user based on the order status and context.
 */
...
export async function handleProcessingOrder(
    from: string,
    state: ConversationDto,
    order_id: number,
): Promise<string[]> {
...
export async function handleSplitBillNumber(
    from: string,
    userMessage: string,
    state: ConversationDto,
): Promise<string[]> {
...
export async function handleWaitingForContacts(
    from: string,
    state: ConversationDto,
    message: Message,
): Promise<string[]> {
...
export async function handleExtraTip(
    from: string,
    userMessage: string,
    state: ConversationDto,
): Promise<string[]> {
 ...
 *
 * Handles the user's payment confirmation process and validates the proof of payment.
 *
 * @param from - The user's unique identifier (WhatsApp ID).
 * @param userMessage - The text message sent by the user, potentially containing payment details or confirmation.
 * @param state - The current state of the user's conversation.
 * @param message - The message object containing potential media or proof of payment.
 * @returns A Promise that resolves to an array of strings representing the messages sent to the user.
 * 
 * Functionality:
 * - Validates the payment proof sent by the user, checking for duplicates, beneficiary details, and payment amount.
 * - Updates the conversation state based on the payment validation results:
 *   - Confirmed payment.
 *   - Overpayment decision.
 *   - Pending balance.
 *   - Invalid payment.
 * - Sends follow-up messages to guide the user based on the payment status.
 * - Handles reminders if no payment proof is received within a given time frame.
 */

export async function handleWaitingForPayment(
    from: string,
    userMessage: string,
    state: ConversationDto,
    message: Message,
...
export async function handleOverpaymentDecision(
    from: string,
    userMessage: string,
    state: ConversationDto,
): Promise<string[]> {
...
export async function handleAwaitingUserDecision(
    from: string,
    userMessage: string,
    state: ConversationDto,
): Promise<string[]> {
